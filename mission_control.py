...
def api_macro_pair(pair_id):
    """
    Per-pair AI analysis for macro.html cards.
    Served from cache (pre-warmed by macro_scheduler every 60 min).
    Pass ?refresh=1 to force a fresh AI call.
    """
    ...
    try:
        force = request.args.get("refresh") == "1"
        data  = get_pair_card_analysis(pair_id, force=force)
        return jsonify({"ok": True, "pair": pair_id, "data": data})
    except Exception as e:
        print(f"[macro_routes] ERROR {request.path}: {e}", flush=True)
        return jsonify({"ok": False, "error": str(e)}), 500
