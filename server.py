...
class PairServer:
    ...
    def _debug_replay(self):
        ...
        try:
            ...
            full_df = full_df.iloc[:-1].copy()  # strip still-forming candle — keep only this one
            total = len(full_df)
            ...
            print(f"[REPLAY DEBUG] full_df[-1]={int(full_df.index[-1].timestamp())} full_df[-2]={int(full_df.index[-2].timestamp())}")
            print(f"[REPLAY DEBUG] full_df[124]={int(full_df.index[124].timestamp())} full_df[125]={int(full_df.index[125].timestamp())} full_df[126]={int(full_df.index[126].timestamp())}")
