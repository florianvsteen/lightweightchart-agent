...
def _build_prompt(events: list[dict]) -> str:
    ...
    return (
        f"You are a macro forex analyst. Analyze these {len(events)} economic events.\n\n"
        f"{event_block}\n\n"
        "For EACH event write 2 sentences. Think like a macro trader — consider:\n"
        "- The current central bank stance (hiking, holding, cutting cycle)\n"
        "- What this data means for rate expectations, not just the number itself\n"
        "- Broader market context: risk-on/off, correlations, upcoming events that interact\n"
        "- If previous >> forecast, the bar is already low — factor that in\n\n"
        "Rules:\n"
        "- Number each response to match the event number\n"
        "- Reference the actual previous/forecast numbers\n"
        "- Name the most affected currency pair(s)\n"
        "- VARY sentence structure — each event should read differently\n"
        "- No generic statements like 'higher is bullish' — explain the macro WHY\n"
        "- No disclaimers, no fluff\n\n"
        "Examples of the macro depth expected:\n"
        "1. With the Fed holding and inflation still sticky, a Core PPI above 0.3% would kill "
        "rate cut hopes entirely — EUR/USD risks a flush lower toward key support. "
        "Soft print below 0.3% reopens the June cut narrative and gives bulls a reason to fade USD.\n"
        "2. BOJ has been jawboning a hike for months — a hold at <0.75% with a hawkish statement "
        "still lifts JPY as markets price the next meeting; a dovish hold sends USD/JPY back above 150.\n"
        "3. Claimant Count has been creeping higher — another beat above 28.6K would confirm the "
        "UK labour market is cracking under high rates, pressuring GBP/USD and giving the MPC cover to cut.\n\n"
        "Now analyze:"
    )
