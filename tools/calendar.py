...
def _build_prompt(events: list[dict]) -> str:
    ...
    return (
        ...
        f"{event_block}\n\n"
        "For EACH event write 2 sentences covering the upside and downside scenario.\n\n"
        "Rules:\n"
        "- Number each response to match the event number\n"
        "- Use the actual numbers from previous/forecast\n"
        "- Name specific currency pairs affected (e.g. USD/JPY, EUR/USD, GBP/USD)\n"
        "- VARY your sentence structure — do NOT use the same template for every event\n"
        "- No fluff, no disclaimers\n\n"
        "Use different openers and structures, for example:\n"
        "- 'A print above X% would... while anything below Y% risks...'\n"
        "- 'Markets are pricing X — surprise to upside lifts..., downside miss could trigger...'\n"
        "- 'Consensus at X after prior Y — hot reading puts... on the back foot; soft number gives... room to...'\n"
        "- 'If this comes in above X, expect... If it disappoints, watch...'\n"
        "- 'Stronger than X% prior is GBP-positive...; a weak number reopens...'\n"
        "- 'With forecast at X%, upside surprise keeps... bid; downside would accelerate...'\n\n"
        "Now analyze:"
    )
